"""
ChromaDB Search Executor — Execução de Busca por Tipo RAG

Módulo especializado na execução das buscas ChromaDB para o pipeline RAG.
Cada tipo de query (SEMANTIC, METADATA, ENTITY, TEMPORAL, CONTENT) possui
estratégia de execução dedicada com filtros e fallbacks apropriados.

Posição no Pipeline: Fase 3 — Busca ChromaDB
"""
...
__all__ = ["ChromaDBSearchExecutor"]


class ChromaDBSearchExecutor:
    """
    ChromaDB Search Executor — Fase 3 do Pipeline RAG

    Executa buscas ChromaDB por tipo de query, construindo filtros dinâmicos,
    aplicando fuzzy matching e normalizando nomes de clientes via lookup no banco.
    Suporta 5 tipos RAG: SEMANTIC, METADATA, ENTITY, TEMPORAL, CONTENT.
    """

    def __init__(self, chromadb_manager: Any, embedding_generator: Any) -> None:
        """
        Inicializa o executor com integrações ChromaDB.

        Args:
            chromadb_manager: Instância ChromaDBStore para consultas
            embedding_generator: Instância EmbeddingGenerator para embeddings semânticos
        """
    ...
    def execute_search(
        ...
        """
        Executa busca ChromaDB de acordo com o tipo de query classificado.

        Args:
            enrichment_result: Resultado do enriquecimento (entidades, contexto)
            classification_result: Resultado da classificação (tipo, estratégia, confiança)
            query: Query original do usuário

        Returns:
            Lista de resultados brutos do ChromaDB para seleção de chunks
        """
    ...
    def _search_term_in_all_chunks(self, search_term: str, limit: int = 50) -> list[dict[str, Any]]:
        ...
        Usado como fallback quando filtros ChromaDB falham.

        Args:
            search_term: Termo exato a buscar
            limit: Número máximo de resultados

        Returns:
            Lista de chunks contendo o termo buscado
        """
        try:
    ...
    def _extract_kt_title_filter(self, enhanced_terms: dict[str, Any]) -> dict[str, Any] | None:
        ...
        Retorna padrões para pós-processamento (ChromaDB não suporta operações complexas de string).
        """
    ...
    def _get_client_variations(self, detected_client: str) -> list[str]:
        ...
        Retorna todos os client_name que fazem fuzzy match com o nome detectado.
        """
        try:
    ...
    def _normalize_client_for_filter(self, detected_client: str) -> str | None:
        ...
        Usa lookup dinâmico no ChromaDB em vez de mapeamentos hardcoded.
        """
    ...
    def _build_video_filters(self, video_references: list[str]) -> dict[str, Any] | None:
        ...
        video_references já contém os nomes reais de vídeos (após fuzzy matching).
        """
        try:
